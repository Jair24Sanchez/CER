// Generador de páginas para países e idiomas
...
<body>
    ...
    <script>
        ...
        function loadReportajes() {
            ...
            const countryReportajes = reportajes.filter(r => 
                r.type === 'irl' && r.subtype === '${countryCode.toLowerCase()}'
            );
            ...
            grid.innerHTML = countryReportajes.map(reportaje => \`
                <div class="reportaje-card">
                    ...
                    <div class="reportaje-meta">
                        <strong>Tipo:</strong> \${reportaje.typeLabel} | 
                        <strong>País:</strong> \${reportaje.subtypeLabel} | 
                        <strong>Fecha:</strong> \${new Date(reportaje.timestamp).toLocaleDateString()}
                    </div>
                </div>
            \`).join('');
        }

        // Cargar reportajes al cargar la página
...
<body>
    ...
    <script>
        ...
        function loadReportajes() {
            const reportajes = JSON.parse(localStorage.getItem('reportajes') || '[]');
            const languageReportajes = reportajes.filter(r => 
                r.type === 'digital' && r.subtype === '${langCode}'
            );
            ...
            grid.innerHTML = languageReportajes.map(reportaje => \`
                <div class="reportaje-card">
                    <div class="reportaje-title">\${reportaje.title}</div>
                    <div class="reportaje-content">\${reportaje.content}</div>
                    <div class="reportaje-meta">
                        <strong>Tipo:</strong> \${reportaje.typeLabel} | 
                        <strong>Idioma:</strong> \${reportaje.subtypeLabel} | 
                        <strong>Fecha:</strong> \${new Date(reportaje.timestamp).toLocaleDateString()}
                    </div>
                </div>
            \`).join('');
    ...
    function generateAllPages() {
        console.log('Generando páginas para países...');
        
        // Generar páginas para países (solo algunos ejemplos)
        ...
        countriesToGenerate.forEach(country => {
            ...
            console.log(`Página generada para ${country.name}: ${country.code}.html`);
        });

        console.log('Generando páginas para idiomas...');
        
        // Generar páginas para idiomas (solo algunos ejemplos)
        ...
        languagesToGenerate.forEach(language => {
            ...
            // En un entorno real, aquí guardarías el archivo
            console.log(`Página generada para ${language.name}: ${language.code}.html`);
        });
    }

    // Ejecutar generación (solo para demostración)
    // generateAllPages();
});
